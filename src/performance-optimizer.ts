...
export class PerformanceOptimizer {
  ...
  analyzePerformanceBottlenecks(structure: CodeStructure[], metrics: CodeMetrics): {
    ...
    const complexFunctions = structure
      ...
      .flatMap(file => 
        file.functions?.filter(func => func && func.complexity > 10) || []
      )
  ...
  private calculateOptimizationScore(
    ...
    if (bottlenecks.length === 0) return 95; // 无瓶颈，性能良好
    ...
    // 根据基础指标调整评分
    if (metrics.averageComplexity > 15) score -= 10;
    if (metrics.totalFiles > 1000) score -= 5;
    if (metrics.totalLines > 50000) score -= 5;
   ...
   * 判断是否为性能关键语言
   */
  private isPerformanceCriticalLanguage(language: string): boolean {
...
`;
  ...
  private getScoreDescription(score: number): string {
    if (score >= 90) return '优秀 - 无需优化';
    if (score >= 80) return '良好 - 轻微优化';
    if (score >= 70) return '中等 - 需要关注';
    if (score >= 60) return '一般 - 需要优化';
    return '较差 - 急需优化';
  }
