...
import { AnalysisResult, CodeStructure, CodeMetrics } from './types.js';
...
export class AIAnalyzer {
  ...
  constructor(apiKey?: string, model: string = 'gpt-4', config?: any) {
...
请使用中文回复，保持专业但易懂的语调。`
   ...
   * 生成代码质量评分
   */
  async generateCodeQualityScore(
    structure: CodeStructure[],
    metrics: CodeMetrics
  ): Promise<{
    overallScore: number;
...
架构特点：
${structure.map(item => 
  `- ${item.path}: ${item.type === 'file' ? item.language : 'directory'} (${item.size} bytes, ${item.lines} lines)`
).join('\n')}
...
代码结构：
${structure.map(item => 
  `- ${item.path}: ${item.type === 'file' ? `${item.language} (${item.lines} lines)` : 'directory'}`
).join('\n')}
...
`;
   ...
   * 解析质量评分响应
   */
  private parseQualityResponse(response: string) {
    // 尝试解析JSON响应
    try {
