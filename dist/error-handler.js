...
export class ErrorHandler {
     ...
     * 安全地执行文件操作
     */
    async safeFileOperation(operation, fallbackValue, context) {
        return this.wrapAsyncOperation(operation, context).then(result => result.result ?? fallbackValue, () => fallbackValue);
    }
    /**
     * 分析和分类错误
     */
...
`;
    ...
    classifyErrors(severity) {
        ...
        const filteredErrors = this.errors.filter(error => severityMap[error.type] === severity);
    ...
    calculateHealthScore() {
        ...
        // 根据错误严重程度扣分
        const criticalErrors = this.errors.filter(e => e.type === 'FileSystemError' || e.type === 'NetworkError').length;
     ...
     * 恢复分析失败的项目
     */
    async recoverFailedAnalysis(originalResult, structure) {
