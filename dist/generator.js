...
export class DocumentGenerator {
    config;
    analysisResult;
    constructor(config, analysisResult) {
        this.config = config;
...
`;
    ...
    generateDetailedStructureMarkdown() {
        const structure = this.analysisResult.structure;
        const files = structure.filter(item => item.type === 'file');
        let markdown = '# 详细项目结构\n\n';
        // 按语言分组
    ...
    generateDetailedDependencyMarkdown() {
        const deps = this.analysisResult.dependencies;
        let markdown = '# 详细依赖分析\n\n';
        // 依赖关系图
        markdown += '## 依赖关系图\n\n';
    ...
    calculateDependencyDepth(file, graph, depthCache) {
        ...
        const maxDepth = Math.max(...dependencies.map(dep => this.calculateDependencyDepth(dep, graph, depthCache)));
...
<body>
    ...
    generateTreeStructure(structure) {
        const directories = structure.filter(item => item.type === 'directory');
        const files = structure.filter(item => item.type === 'file');
        let tree = '';
        // 添加根目录
